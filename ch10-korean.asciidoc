...
// 번역자노트
// cryptozeny
// 1. 번역하며 이해가 안 가는 부분은 다음과 같이 하이라이트 처리하였다.
// #이해안가는부분#
// 2. 번역자가 해설을 꼭 해야하는 부분은 다음과 같이 red 처리하였다.
// [red]#편집자주: 할말#
//
[[mining]]
...
채굴 및 합의 그리고 목적의 관점에서 "채굴"이라는 용어는 다소 오해의 소지가 있다. 마치 그것은 귀금속의 채굴 과정을 연상시키며 비트코인의 채굴을 보상금과 각 블록 내의 비트코인에만 집중하고 있다. 물론 보상금 때문에 채굴 작업이 활발하게 이루어지긴 하지만 채굴의 주된 목적은 보상금 혹은 새로운 코인의 생성이 아니다. 여러분이 채굴을 단지 코인을 생성하는 프로세스라고만 인식한다면 프로세스의 수단(인센티브)을 목적으로 잘못 판단하고 있는 셈이다. 채굴은 분산화된 #클리어링하우스(clearinghouse)#의 주요 프로세스로써 채굴에 의해서 거래를 검증하고 승인한다. 그렇게 채굴은 비트코인을 특별하게 만들어 주는 발명품이자 P2P 기반 전자화폐의 근간이 되는 탈중앙화 보안 메커니즘이다.
...
채굴은 _비트코인 시스템을 보호하고_ _중앙화된 신뢰기관 없이_ 네트워크 차원의 _합의를 도출_ 할 수있게 해준다. 새로 발행된 코인과 거래 수수료의 보상은 인센티브의 관점으로, 채굴자가 네트워크의 보안을 담당하는 동시에 화폐의 공급을 동시에 구현하는 제도이다.
...
채굴의 목적은 새로운 비트코인을 만들어내는 것이 아니다. 그것은 인센티브 시스템이다. 채굴은 비트코인의 _보안성_ 이 _탈중앙화_ 되는 메커니즘이다.
====
